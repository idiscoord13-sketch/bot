...
//error_log("role id is " . $user_role->id);
//error_log("role id is " );
...
switch ($user_role->id) {
	...
	case ROLE_Shahzadeh:
		...
		if ($user->dead()) {
			$keyboard = []; 
			if ($server->setUserId(ROLE_Shahzadeh)->getMetaUser('select') == 0) {
				
				$server->setUserId(ROLE_Shahzadeh)->updateMetaUser('select', 1 );

				$message .= '⚖️ مأموریت : انتخاب کنید میخواهید چه کسی در رای گیری روز بعد انتخاب دوگانه داشته باشد.' . "\n";
				foreach ($users_server as $item) {

					if ($item->check($user)) {

						$keyboard[][] = $telegram->buildInlineKeyboardButton('⚖️ ' . $item->get_name(), '', $day . '/server-' . $server->league_id . '-shahzadeh-' . $server->getId() . '-' . $item->getUserId());
		...
		{
			$message .= "🗞 ماموریت: تو وارث تاجی؛ حضورت بازی را حفظ میکند، مرگت بازی را از مسیر خود خارج میسازد."."\n"; 
			$message .= '💬 چت : غیرفعال ' . "\n";
		}
		$message .= '🌞 روز 👈🏻 40 ثانیه' . "\n";
    ...
    case ROLE_Terrorist:
        ...
        break;
    case ROLE_MineLayer:
        error_log("ROLE_MineLayer power");
        if ($day == 1) {
        }
        error_log("ROLE_MineLayer power1894");
        $select_minelayer = $selector->select(ROLE_MineLayer);
        error_log("select_minelayer " . $select_minelayer->getUserId());
        ...
        if ($user->dead()) {
            ...
            error_log("before power");
            $power = $selector->getInt()->select(ROLE_MineLayer, 'mine');
            error_log("mine is $power");
//            $message .= '💣 تعداد مین باقیمانده : ' . $power . "\n";
            if ($select_minelayer->getUserId() > 0) {
                $message .= '🗞 مأموریت : یک خانه از 1 تا 10 برای مین‌گذاری انتخاب کنید ' . "\n";
            }
            $message .= '💬 چت : غیرفعال ' . "\n";
        ...
        if ($user->check($bazpors_select) && !$user->is($dozd_select) && $select_minelayer->getUserId() > 0) {
            error_log("sending message");
//            $user->setKeyboard(
            $user->setKeyboard(
                $telegram->buildInlineKeyBoard([
                    [
                        $telegram->buildInlineKeyboardButton('1⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-1'),
                        $telegram->buildInlineKeyboardButton('2⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-2'),
                        $telegram->buildInlineKeyboardButton('3⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-3'),
                        $telegram->buildInlineKeyboardButton('4⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-4'),
                        $telegram->buildInlineKeyboardButton('5⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-5'),
                    ],
                    [
                        $telegram->buildInlineKeyboardButton('6⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-6'),
                        $telegram->buildInlineKeyboardButton('7⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-7'),
                        $telegram->buildInlineKeyboardButton('8⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-8'),
                        $telegram->buildInlineKeyboardButton('9⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-9'),
                        $telegram->buildInlineKeyboardButton('🔟', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-10'),
                    ]
                ])
            );
//            )->SendMessageHtml();

            $temp = '💣♂ سلام ' . "<b>{$select_minelayer->get_name()}</b>" . "\n";
            $temp .= 'شما هدف مین‌گذاری قرار گرفته اید و جان شما در خطر است .' . "\n";
            $temp .= 'تنها در صورت پیدا کردن مین ، زنده خواهید ماند .' . "\n \n \n";
            $temp .= 'برای خنثی کردن ۳ خانه را انتخاب کنید .👇🏻';


            $select_minelayer->setKeyboard(
                $telegram->buildInlineKeyBoard([
                    [
                        $telegram->buildInlineKeyboardButton('1⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-1'),
                        $telegram->buildInlineKeyboardButton('2⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayer . '-' . $server->getId() . '-2'),
...
//        error_log("ROLE_MineLayerMafia power");
        if ($day == 1) {
            add_server_meta($server->getId(), 'mine', 5, ROLE_MineLayerMafia);
        }

//        $mine = $selector->getInt()->select(ROLE_MineLayerMafia, 'mine', false);
//        error_log("ROLE_MineLayerMafia power1894");
        $select_minelayer = $selector->select(ROLE_MineLayerMafia);
//        error_log("select_minelayer " . $select_minelayer->getUserId());
        ...
        $role = $server->get_priority();
        if ($user->check($bazpors_select) && apply_filters('filter_mafia', $user->getUserId()) && $role->id == $user_role->id) {
            $message .= "امشب شما به جای پدرخوانده یک نفر رو انتخاب میکنید تا کشته شود." . "\n \n";
            $message .= $server->showTeam($user->getUserId()) . "\n";
                foreach ($users_server as $item) {
...
//            error_log("mine is $power");
            ...
            if(!$user->is($bazpors_select) && !$user->dead())
            {
                $message .= $server->showTeam($user->getUserId());
            }
...
//            $user->setKeyboard(
            $user->setKeyboard(
                $telegram->buildInlineKeyBoard([
                    [
                        $telegram->buildInlineKeyboardButton('1⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-1'),
                        $telegram->buildInlineKeyboardButton('2⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-2'),
                        $telegram->buildInlineKeyboardButton('3⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-3'),
                        $telegram->buildInlineKeyboardButton('4⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-4'),
                        $telegram->buildInlineKeyboardButton('5⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-5'),
                    ],
                    [
                        $telegram->buildInlineKeyboardButton('6⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-6'),
                        $telegram->buildInlineKeyboardButton('7⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-7'),
                        $telegram->buildInlineKeyboardButton('8⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-8'),
                        $telegram->buildInlineKeyboardButton('9⃣', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-9'),
                        $telegram->buildInlineKeyboardButton('🔟', '', $day . '/server-' . $server->league_id . '-' . ROLE_MineLayerMafia . '-' . $server->getId() . '-10'),
                    ]
                ])
            );
//            )->SendMessageHtml();
            ...
            $power = intval($selector->getInt()->select(ROLE_MineLayerMafia, 'mine'));
        ...
        if ($user->check($bazpors_select)) {
            ...
            foreach ($users_server as $item) {
                ...
                if ($item->check($user)&& $item->get_role()->group_id != 3) {
                    ...
                    $keyboard[] = [
                        $telegram->buildInlineKeyboardButton('⚰️' . $item->get_name(), '', $day . '/server-' . $server->league_id . '-gorkan-' . $server->getId() . '-' . $item->getUserId())
                    ];
