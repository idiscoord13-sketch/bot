...
class wpdb
{
    ...
    {
        return $this->objectBuilder()->query( $query );
    }
    ...
    {
        if ( ! str_contains( strtolower( $query ), 'limit 1' ) ) $query .= "  LIMIT 1";
        return $this->objectBuilder()->query( $query )[ 0 ];
    }
     ...
     * @throws Exception
     */
    public function get_var( $query, int $default = 0 )
    {
        if ( $this->get_row( 'SELECT IF( EXISTS( ' . $query . ' ), 1, 0) as `exists`' ) == 1 )
        {
     ...
     *
     * @return array|$this Contains the returned rows from the select query.
     * @throws Exception
     */
     ...
     *
     * @return $this new mysqlidb object
     */
