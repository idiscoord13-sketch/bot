...
try
{
	...
	include INCLUDE_DIR.'/handler2.php';
    ...
    if ( $type == 'private' && isset( $message->reply_to_message ) )
    {
        ...
        require_once BOT_DIR . '/reply_to_message.php';
    ...
    if ( isset( $update->callback_query ) )
    {
        ...
        if ( $data[ 0 ] == 'create_game' )
        {
            ...
            require_once BOT_DIR . '/create_game.php';
    ...
    if ( $type == 'private' && isset( $message->from ) )
    {
        ...
        require_once BOT_DIR . '/friend_server.php';

        require_once BOT_DIR . '/private_chat.php';
    }

    if ( $type == 'supergroup' && isset( $message->reply_to_message ) )
    {

        require_once BOT_DIR . '/group_reply_to_message.php';
    ...
    if ( $type == 'private' && isset( $update->callback_query ) )
    {
        ...
        require_once BOT_DIR . '/callback_query.php';
    }
    ...
    if ( isset( $update->inline_query ) )
    {
        ...
        require_once BOT_DIR . '/inline_query.php';
    }
    ...
    {
        $data = explode( ' ', $text );
        require_once BOT_DIR . '/group_private.php';
    }
    ...
    if ( ( $type == 'supergroup' || $type == 'group' ) && isset( $update->callback_query ) )
    {
        ...
        $data = explode( '-', $Data );
        require_once BOT_DIR . '/group_callback_query.php';
...
{
    // ساخت متن خطا برای ارسال به ادمین
    $adminMessage = "<i>ERROR LINE: {" . $e->getLine() . "}</i>\n\n";
    $adminMessage .= "<i>ERROR User: {" . ($chat_id ?? $chatid) . "}</i>\n\n";
    $adminMessage .= "<u>ERROR ON FILE: {" . $e->getFile() . "}</u>\n\n";
    $adminMessage .= "<b>CONTACT ERROR: [" . $e->getMessage() . "]</b>";

    // ذخیره خطا در فایل error_log.txt
    $logMessage  = "==============================\n";
    $logMessage .= "DATE: " . date('Y-m-d H:i:s') . "\n";
    $logMessage .= "USER CHAT ID: " . ($chat_id ?? $chatid) . "\n";
    ...
    // ارسال به ادمین تلگرام
    SendMessage(5889394964, $adminMessage, null, null, 'html');

    // پیام به کاربر
    $userMessage = '🔴 متاسفانه خطایی رخ داد،' . "\n\n" . '⚠️ گزارش خطا برای پشتیبانی ارسال شد. لطفا مجددا تلاش کنید🙏';
    SendMessage($chat_id ?? $chatid, $userMessage);
}
